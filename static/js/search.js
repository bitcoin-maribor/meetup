document.addEventListener("DOMContentLoaded", () => {
  ...
  const loadIndex = async () => {
    ...
    try {
      const rawDocs = window.searchIndex;
      if (!Array.isArray(rawDocs)) throw new Error("Search index not available");
      // Transparent sections can index the same page in both the archive and home feed.
      // MiniSearch requires a unique ID for each document.
      const documentsByUrl = new Map();
      for (const doc of rawDocs) {
        if (doc.url && (doc.title || doc.body)) {
          documentsByUrl.set(doc.url, {
            id: doc.url, title: doc.title || "", body: doc.body || ""
          });
        }
      }
      ...
      root.searchIndex.addAll([...documentsByUrl.values()]);
